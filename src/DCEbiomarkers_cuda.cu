...
/*  for(int i=0;i<NX;i++)  
	...
	int *dev_mriSizes;
	float *dev_time,*dev_AIF,*dev_Ct,*dev_vp,*dev_ktrans,*dev_kep,*dev_converged;
  ...
  printf("---> Kernel execution...\n");  
	...
	if(cudaThreadSetLimit(cudaLimitMallocHeapSize,2*total)){
   	fprintf(stderr, "Cuda error: Failed to get Thread Limit Malloc Heap Size\n");
   	return 0;
	}
	for(int i=0;i<mriSizes[2]/grid.z;i++){
		NLLSQkernel<<<grid,block>>>(nsd,nt,dev_mriSizes,difft,dev_Ct,dev_time,dev_AIF,
																dev_Cp,dev_fftCp,dev_vp,dev_ktrans,dev_kep,dev_converged,i);
...
//    for(int pz=0;pz<mriSizes[2];pz++){
  ...
  */
      ...
      while( its<MAXIT && (norm(dB,NPARAMS)>1e-6||dr>1e-6) && kep0>1e-9 ){        
     		its++;
        for(int l=0;l<nt;l++)
          Exp[l] *= time[l];
  ...
  */
  ...
  /*
		  if(px==0&&py==0&&pz==0) 
		    printf("it: %d %f %f %f %f %f %f\n",its,dr,r_new,norm(dB,NPARAMS),beta[0],beta[1],beta[2]);
  */
        r_old = r_new;
      }
      if(its!=MAXIT)	
